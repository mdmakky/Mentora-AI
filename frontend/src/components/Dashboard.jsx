...
const Dashboard = () => {
  ...
  const [form, setForm] = useState({ name: '', year: new Date().getFullYear(), term: 'Spring', is_current: true });
  ...
  return (
    <div className="app-content animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Semesters', value: semesters.length, icon: BookOpen, color: 'bg-emerald-50 text-emerald-600' },
          { label: 'Documents', value: '—', icon: FileText, color: 'bg-blue-50 text-blue-600' },
          { label: 'Chat Sessions', value: '—', icon: MessagesSquare, color: 'bg-violet-50 text-violet-600' },
          { label: 'Study Streak', value: '🔥 0 days', icon: TrendingUp, color: 'bg-amber-50 text-amber-600' },
        ].map((stat) => (
